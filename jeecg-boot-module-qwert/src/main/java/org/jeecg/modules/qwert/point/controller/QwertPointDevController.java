...
//		 String proType = jsonConInfo.getString("proType");
		 ...
		 String rsl = resList.toString();
		 if(!resList.toString().equals("[{}]")){
			 ...
			 for(int k=0;k<qptList.size();k++){
				 QwertPointTarget qpt= qptList.get(k);
				 QwertPointTarget2 qpt2=new QwertPointTarget2();
				 qpt2.setTargetNo(qpt.getTargetNo());
				 qpt2.setTargetName(qpt.getTargetName());
				 qpt2.setTargetValue("");
				 qpt2List.add(qpt2);
			 }
		 }
		 return Result.ok(qpt2List);
	 }
